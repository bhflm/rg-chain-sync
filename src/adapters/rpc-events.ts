...
import { CommitmentBatchEventArgs, NullifiersEventArgs, UnshieldEventArgs, ShieldEventArgs } from "../types/rpc-events";
...
export function adaptParsedCommitmentBatch(
  ...
  if (!args || typeof args.treeNumber === 'undefined' || !Array.isArray(args.hash)) {
      console.error("RPC Adapter: Invalid args for CommitmentBatch", log);
      throw new Error("Invalid CommitmentBatch event args");
  }

  const commitments = (args.hash || []).map((hash: Hash, index: number) => ({ // Now args.hash is typed
      hash: hash,
...
export function adaptParsedNullifiers(
  ...
  const args = log.args as NullifiersEventArgs; // <--- TYPE ASSERTION @TODO: Remove this, find a clever way of infer the type as in subsquid
   ...
   if (!args || typeof args.treeNumber === 'undefined' || !Array.isArray(args.nullifier)) {
...
export function adaptParsedShield(
  ...
  if (!args || !Array.isArray(args.commitments) || typeof args.startPosition === 'undefined') {
      console.error("RPC Adapter: Invalid args for Shield", log);
      throw new Error("Invalid Shield event args");
  }

  const commitments = (args.commitments || []).map((preimage, index: number) => {
       // TODO: Replace placeholder with actual hashing!
       const hash = ByteUtils.formatToByteLength('0x' + Math.random().toString(16).slice(2), ByteLength.UINT_256) as Hash;
