...
import {
  ...
  isNullifiersEntry,
} from "../src/types/data-entry"; // Import new types
...
describe("SubsquidSource", () => {
  ...
  it("should fetch the first Nullifier event at or after block 14755920", async () => {
    ...
    const maxBlocksToScan = 5000n; // Limit search range to avoid excessively long test
    const endScanBlock = targetBlock + maxBlocksToScan;

    console.log(
      `[Test Subsquid Source] Starting search for Nullifier at/after block ${targetBlock}...`,
    );
    ...
    for await (const entry of iterator) {
      console.log(
        `[Test Subsquid Source] Processing entry: Type=${entry.type}, Block=${entry.blockNumber}`,
      );

      // Stop if we scan too far
      if (entry.blockNumber > endScanBlock) {
        console.log(
          `[Test Subsquid Source] Scanned up to block ${entry.blockNumber}, stopping search.`,
        );
        break;
      }
      ...
      if (entry.blockNumber >= targetBlock && isNullifiersEntry(entry)) {
        ...
        console.log(
          `[Test Subsquid Source] Found Nullifiers entry at block ${entry.blockNumber}, tx ${entry.transactionHash}`,
        );
        ...
        assert.ok(
          ...
          `Block number ${entry.blockNumber} >= ${targetBlock}`,
        );
        assert.ok(
        ...
        ); // Adapter makes one entry per nullifier
        ...
        assert.strictEqual(
          nullifierHex.length,
          66,
          `Nullifier hex length: ${nullifierHex.length}`,
        );
        ...
        if (entry.blockNumber === targetBlock) {
          ...
          assert.ok(
            knownNullifiersInBlock.includes(nullifierHex),
            `Nullifier ${nullifierHex} not expected in block ${targetBlock}`,
          );
          console.log(
            `[Test Subsquid Source] Nullifier ${nullifierHex} matches known value for block ${targetBlock}.`,
          );
        }

        console.log(
          "[Test Subsquid Source] Validation successful. Stopping iteration.",
        );
    ...
    assert.ok(
      foundMatch,
      `Test failed: No Nullifiers event found and validated at or after block ${targetBlock} within ${maxBlocksToScan} blocks`,
    );
